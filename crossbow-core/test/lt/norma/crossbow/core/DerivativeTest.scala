...
class DerivativeTest extends FunSuite {
  ...
  test("isExpired") {
    ...
    assert {
      !derivative.isExpired(new DateMidnight(2011, 11, 20))
    }
    assert {
      !derivative.isExpired(new DateMidnight(2011, 11, 29))
    }
    assert {
      derivative.isExpired(new DateMidnight(2011, 11, 30))
    }
    assert {
      derivative.isExpired(new DateMidnight(2011, 12, 1))
    }
    assert {
      derivative.isExpired(new DateMidnight(2012, 1, 1))
    }
  }
