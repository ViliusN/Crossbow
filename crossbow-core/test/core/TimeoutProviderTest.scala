...
class TimeoutProviderTest extends FunSuite {
  test("dependencies") {
    ...
    expect(Set.empty) { provider.dependencies }
  }
  ...
  test("timeout") {
    ...
    val listener = Listener {
      case Timeout(sr) =>
        approx(DateTime.now.getMillis, 100) { start.endTime.getMillis }
        assert { sr eq start }
        received = true;
      case _ =>
        fail
  ...
  test("timeout - two requests") {
    ...
    val listener = Listener {
      case Timeout(`start1`) =>
        approx(DateTime.now.getMillis, 100) { start1.endTime.getMillis }
        received1 = true;
      case Timeout(`start2`) =>
        approx(DateTime.now.getMillis, 100) { start2.endTime.getMillis }
        received2 = true;
      case _ =>
        fail
    }
  ...
  test("Start - alternative constructor") {
    ...
    approx(DateTime.now.getMillis + 1000, 100) { start.endTime.getMillis }
  }
}
