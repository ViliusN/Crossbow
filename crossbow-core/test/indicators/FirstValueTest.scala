...
class FirstValueTest extends FunSuite {
  test("name") {
    ...
    expect("First value of T") { indicator.name }
  }
  ...
  test("value - String") {
    val target = new Variable[String] { override def name = "T" }
    val indicator = new FirstValue(target)
    val list = new IndicatorList(indicator)
    expect(None) { indicator() }
    list.send(EmptyMessage)
  ...
  test("initial value") {
    val target = new Variable(55) { override def name = "T" }
    val indicator = new FirstValue(target)
    val list = new IndicatorList(indicator)
    expect(Some(55)) { indicator() }
    target.set(8)
    list.send(EmptyMessage)
    expect(Some(55)) { indicator() }
  }
