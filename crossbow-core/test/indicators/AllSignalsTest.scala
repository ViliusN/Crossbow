...
class AllSignalsTest extends FunSuite {
  ...
  test("name - multiple targets") {
    val target1 = new VariableSignal { override def name = "T1" }
    val target2 = new VariableSignal { override def name = "T2" }
    val target3 = new VariableSignal { override def name = "T3" }
    val signal = new AllSignals(target1, target2, target3)
    expect("AllSignals(T1; T2; T3)") { signal.name }
  }
  test("name - one target") {
    val target = new VariableSignal { override def name = "T1" }
    val signal = new AllSignals(target)
    expect("AllSignals(T1)") { signal.name }
  }
  test("name - no targets") {
    val signal = new AllSignals()
    expect("AllSignals()") { signal.name }
  }
  ...
  test("dependencies - multiple targets") {
    ...
    val signal = new AllSignals(target1, target2, target3)
    expect(Set(target1, target2, target3)) { signal.dependencies }
  }
  test("dependencies - one target") {
    val target1 = new VariableSignal
    val signal = new AllSignals(target1)
    expect(Set(target1)) { signal.dependencies }
  }
  test("dependencies - no targets") {
    val signal = new AllSignals()
    expect(Set()) { signal.dependencies }
  }
